library(dplyr)
library(ggpubr)
load(file = "data/RC_filered.rda")
source("../code/function.R")

upregion_20_7 <- c("AA1", "AC1", "AG1", "CA1", "GA1")
downregion_2_20 <- c("GA1", "GC1", "TC1", "TT1")
RC_filered$ss_type <- tolower(RC_filered$ss_type)
RC_lst <- RC_filered %>%
  filter((ss_type %in% c("normal", "exonic", "intronic"))) %>%
  dplyr::filter(!(region == "-50_-21" & (XYmer %in% upregion_20_7))) %>%
  dplyr::filter(!(region == "+21_+45" & (XYmer %in% downregion_2_20))) %>%
  mutate(region = if_else(region %in% c("-50_-21", "-20_-7"), "-50_-7",
    if_else(region %in% c("+2_+20", "+21_+45"), "+2_+45", region)
  )) %>%
  group_by(XYmer, region) %>%
  group_split()
### T-test
if (!file.exists("data/AltAss_all_region.csv")) {
  stat.test.lst <- lapply(RC_lst, function(x) {
    Group <- unique(x[["ss_type"]])
    res <- tibble()
    for (i in 1:(length(Group) - 1)) {
      for (j in 2:length(Group)) {
        if (i != j) {
          print(paste(unique(x[["XYmer"]]), paste(Group[i], Group[j], sep = "-"), sep = ": "))
          g1 <- x[["Separation"]][x[["ss_type"]] == Group[i]]
          g2 <- x[["Separation"]][x[["ss_type"]] == Group[j]]
          # print(check_var_equality(g1, g2))
          if (check_var_equality(g1, g2)) {
            p <- t.test(g1, g2, paired = TRUE)$p.value
            method <- "T-test"
          } else {
            p <- pairwise.t.test(c(g1, g2),
              rep(c(Group[i], Group[j]),
                each = length(g1)
              ),
              paired = TRUE
            )$p.value[, 1]
            method <- "Pairwise-T-test"
          }
          res <- bind_rows(
            res,
            tibble( # region = unique(x[["region"]]),
              region = unique(x[["region"]]),
              XYmer = unique(x[["XYmer"]]),
              group1 = Group[i], group2 = Group[j],
              p = p, method = method
            )
          )
        }
      }
    }
    # return(res %>%
    #          mutate(p.adj = p.adjust(p, method = "holm")))
    return(res)
  })
  stat.test.df <- do.call(bind_rows, stat.test.lst) %>%
    mutate(
      p.format = sapply(p, pValueFormat),
      p.adj = p.adjust(p, method = "fdr"),
      p.signif = symnum(p,
        cutpoints = c(0, 0.0001, 0.001, 0.01, 0.05, Inf),
        symbols = c("****", "***", "**", "*", "ns")
      )
    )
  readr::write_csv(stat.test.df, file = "data/AltAss_all_region.csv")
} else {
  stat.test.df <- readr::read_csv("data/AltAss_all_region.csv")
}

table(stat.test.df$region, stat.test.df$method)
# Pairwise-T-test T-test
# -50_-7              12     36
# -6_+1                3     45
# +2_+45              34     14



my_comparisons <- list(
  c("exonic", "intronic"),
  c("exonic", "normal"),
  c("intronic", "normal")
)
gg_lst <- lapply(RC_lst, function(x) {
  ggplot(x, aes(ss_type, Separation)) +
    # 添加柱
    stat_summary(
      mapping = aes(fill = ss_type),
      fun = mean, geom = "bar",
      fun.args = list(mult = 1), width = 0.7
    ) +
    # 添加误差线
    stat_summary(
      fun.data = mean_sdl, fun.args = list(mult = 1),
      geom = "errorbar", width = 0.2
    ) +
    facet_grid(~region1) +
    stat_compare_means(
      comparisons = my_comparisons,
      label = "p.signif",
      label.y = c(1.3, 1.5, 1.7),
      method = "t.test",
      p.adjust.method = "fdr",
      # paired = TRUE
    ) +
    # 添加x，y轴名
    labs(x = "Acceptor splicing mode", y = "Separability δ") +
    # 坐标轴延伸，确保图形元素覆盖至坐标
    scale_y_continuous(expand = c(0, 0), limits = c(0, 1.9)) +
    # 主题类型
    theme_classic() +
    # 设置主题
    theme(
      panel.background = element_rect(fill = "white", colour = "black", linewidth = 0.25), # 填充框内主题颜色，边框颜色和边框线条粗细
      axis.line = element_line(colour = "black", linewidth = 0.25), # x,y轴颜色，粗细
      axis.title = element_text(size = 13, color = "black"), # x,y轴名设置
      axis.text = element_text(size = 12, color = "black"), # x,y轴文本设置
      axis.text.x = element_text(angle = 45, hjust = 1),
      legend.position = "none"
    )
})


g1 <- c("CC1", "GG1", "CG1", "GC1", "CT1", "TC1")
g2 <- c("AA1", "AC1")
g3 <- c("AT1", "TA1", "TT1")
g4 <- c("AG1", "CA1", "GA1", "GT1", "TG1")
RC_lst <- RC_filered %>%
  filter((ss_type %in% c("normal", "exonic", "intronic"))) %>%
  dplyr::filter(!(region == "-50_-21" & (XYmer %in% upregion_20_7))) %>%
  mutate(region = if_else(region %in% c("-50_-21", "-20_-7"), "-50_-7",
    if_else(region %in% c("+2_+20", "+21_+45"), "+2_+45", region)
  ))

gg1 <- ggplot(
  RC_lst %>%
    dplyr::filter(region == "-50_-7" &
      XYmer %in% g1),
  aes(ss_type, Separation)
) +
  # 添加柱
  stat_summary(
    mapping = aes(fill = ss_type),
    fun = mean, geom = "bar",
    fun.args = list(mult = 1), width = 0.7
  ) +
  # 添加误差线
  stat_summary(
    fun.data = mean_sdl, fun.args = list(mult = 1),
    geom = "errorbar", width = 0.2
  ) +
  facet_grid(~XYmer) +
  stat_pvalue_manual(
    stat.test.df %>%
      filter(region == "-50_-7" &
        XYmer %in% g1),
    y.position = c(1.5, 1.7, 1.9),
    label = "p.signif"
  ) +
  # 添加x，y轴名
  labs(x = "Acceptor splicing mode", y = "Separability δ") +
  # 坐标轴延伸，确保图形元素覆盖至坐标
  scale_y_continuous(expand = c(0, 0), limits = c(0, 2.0)) +
  # 主题类型
  theme_classic() +
  # 设置主题
  theme(
    panel.background = element_rect(fill = "white", colour = "black", linewidth = 0.25), # 填充框内主题颜色，边框颜色和边框线条粗细
    axis.line = element_line(colour = "black", linewidth = 0.25), # x,y轴颜色，粗细
    axis.title = element_text(size = 13, color = "black"), # x,y轴名设置
    axis.text = element_text(size = 12, color = "black"), # x,y轴文本设置
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = "none"
  )

gg2 <- ggplot(
  RC_lst %>%
    filter(region == "-50_-7" &
      XYmer %in% g2),
  aes(ss_type, Separation)
) +
  # 添加柱
  stat_summary(
    mapping = aes(fill = ss_type),
    fun = mean, geom = "bar",
    fun.args = list(mult = 1), width = 0.7
  ) +
  # 添加误差线
  stat_summary(
    fun.data = mean_sdl, fun.args = list(mult = 1),
    geom = "errorbar", width = 0.2
  ) +
  facet_grid(~XYmer) +
  stat_pvalue_manual(
    stat.test.df %>%
      filter(region == "-50_-7" &
        XYmer %in% g2),
    y.position = c(1.0, 1.2, 1.4),
    label = "p.signif"
  ) +
  # 添加x，y轴名
  labs(x = "Acceptor splicing mode", y = "Separability δ") +
  # 坐标轴延伸，确保图形元素覆盖至坐标
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1.5)) +
  # 主题类型
  theme_classic() +
  # 设置主题
  theme(
    panel.background = element_rect(fill = "white", colour = "black", linewidth = 0.25), # 填充框内主题颜色，边框颜色和边框线条粗细
    axis.line = element_line(colour = "black", linewidth = 0.25), # x,y轴颜色，粗细
    axis.title = element_text(size = 13, color = "black"), # x,y轴名设置
    axis.text = element_text(size = 12, color = "black"), # x,y轴文本设置
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = "none"
  )

gg3 <- ggplot(
  RC_lst %>%
    filter(region == "-50_-7" &
      XYmer %in% g3),
  aes(ss_type, Separation)
) +
  # 添加柱
  stat_summary(
    mapping = aes(fill = ss_type),
    fun = mean, geom = "bar",
    fun.args = list(mult = 1), width = 0.7
  ) +
  # 添加误差线
  stat_summary(
    fun.data = mean_sdl, fun.args = list(mult = 1),
    geom = "errorbar", width = 0.2
  ) +
  facet_grid(~XYmer) +
  stat_pvalue_manual(
    stat.test.df %>%
      filter(region == "-50_-7" &
        XYmer %in% g3),
    y.position = c(1.85, 2.05, 2.2),
    label = "p.signif"
  ) +
  # 添加x，y轴名
  labs(x = "Acceptor splicing mode", y = "Separability δ") +
  # 坐标轴延伸，确保图形元素覆盖至坐标
  scale_y_continuous(expand = c(0, 0), limits = c(0, 2.3)) +
  # 主题类型
  theme_classic() +
  # 设置主题
  theme(
    panel.background = element_rect(fill = "white", colour = "black", linewidth = 0.25), # 填充框内主题颜色，边框颜色和边框线条粗细
    axis.line = element_line(colour = "black", linewidth = 0.25), # x,y轴颜色，粗细
    axis.title = element_text(size = 13, color = "black"), # x,y轴名设置
    axis.text = element_text(size = 12, color = "black"), # x,y轴文本设置
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = "none"
  )

gg4 <- ggplot(
  RC_lst %>%
    filter(region == "-50_-7" &
      XYmer %in% g4),
  aes(ss_type, Separation)
) +
  # 添加柱
  stat_summary(
    mapping = aes(fill = ss_type),
    fun = mean, geom = "bar",
    fun.args = list(mult = 1), width = 0.7
  ) +
  # 添加误差线
  stat_summary(
    fun.data = mean_sdl, fun.args = list(mult = 1),
    geom = "errorbar", width = 0.2
  ) +
  facet_grid(~XYmer) +
  stat_pvalue_manual(
    stat.test.df %>%
      filter(region == "-50_-7" &
        XYmer %in% g4),
    y.position = c(1.3, 1.5, 1.7),
    label = "p.signif"
  ) +
  # 添加x，y轴名
  labs(x = "Acceptor splicing mode", y = "Separability δ") +
  # 坐标轴延伸，确保图形元素覆盖至坐标
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1.8)) +
  # 主题类型
  theme_classic() +
  # 设置主题
  theme(
    panel.background = element_rect(fill = "white", colour = "black", linewidth = 0.25), # 填充框内主题颜色，边框颜色和边框线条粗细
    axis.line = element_line(colour = "black", linewidth = 0.25), # x,y轴颜色，粗细
    axis.title = element_text(size = 13, color = "black"), # x,y轴名设置
    axis.text = element_text(size = 12, color = "black"), # x,y轴文本设置
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.position = "none"
  )

library(cowplot)
up_row <- plot_grid(gg1, gg2, rel_widths = c(6, 2), labels = c("A", "B"))
bottom_row <- plot_grid(gg3, gg4, rel_widths = c(3, 5), labels = c("C", "D"))
gg <- plot_grid(up_row, bottom_row, ncol = 1)

ggsave(gg,
  filename = "figures/alt_subset_UpRegion.pdf",
  width = 16, height = 8
)
